...
def create_playlist(
    ...
    async def _run():
        ...
        if not user or not user.spotify_access_token:
            typer.echo("Not authenticated. Run 'auth' first.")
            raise typer.Exit(1)
        ...
        if not digest or not digest.track_list:
        ...
        if not track_uris:
            typer.echo("No Spotify tracks in digest.")
            raise typer.Exit(1)

        # Create playlist via /me/playlists
        r = _httpx.post(
