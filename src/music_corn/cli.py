...
def create_playlist(
    ...
    async def _run():
        ...
        from music_corn.taste.spotify_client import get_fresh_token

        # Use console token (from developer.spotify.com) — has full write access
        # Falls back to OAuth token if console token not set or expired
        token = settings.spotify_console_token
        if token:
            # Verify it's still valid
            r = _httpx.get("https://api.spotify.com/v1/me",
                           headers={"Authorization": f"Bearer {token}"})
            if r.status_code != 200:
                typer.echo("Console token expired. Get a new one from developer.spotify.com")
                typer.echo("Update SPOTIFY_CONSOLE_TOKEN in .env")
                raise typer.Exit(1)
        else:
            token = await get_fresh_token(email)
            if not token:
        ...
        # Get user ID
        me = _httpx.get("https://api.spotify.com/v1/me", headers=headers).json()
        ...
        if r2.status_code != 201:
            typer.echo(f"Failed to add tracks: {r2.status_code}")
            raise typer.Exit(1)
